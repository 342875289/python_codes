...
p1 = re.compile(r'","')
p2 = re.compile(r'<li>')
p_item = re.compile(r'href="((.)+)">((.)+)</a')
#招聘会
p_item_detail_time1 = re.compile(r'(\d){4}-(\d){2}-(\d){2} ((.){11})')
p_item_detail_location1 = re.compile(r'举办地址.<span>((.)+)</span>')
data_after_split=p1.split(data)
...
for i in range(num_split):
    ...
    if(str_data.find(str_date)!=-1):
     ...
     #   print(str(i)+" OK")
        ...
        a_day_data_after_split=p2.split(str_data)
        num_a_day_split = len(a_day_data_after_split)
    #    print("场数:"+str(num_a_day_split-1))
        for j in range(1,num_a_day_split):
            a_day_a_piece = a_day_data_after_split[j]
            link = "http://jiangnan.91job.gov.cn"+p_item.search(a_day_a_piece).group(1)
            enterpriseName = p_item.search(a_day_a_piece).group(3)
    #        print(link)
